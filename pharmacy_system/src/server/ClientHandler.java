...
public class ClientHandler implements Runnable {
    ...
    private String processRequest(String requestXml) {
        try {
            ...
            switch (action) {
                ...
                case "GET_PENDING_USERS": return handleGetPendingUsers();
                case "APPROVE_USER": return handleApproveUser(root);
                case "REJECT_USER": return handleRejectUser(root);
                ...
                case "GET_PENDING_PHARMACIES": return handleGetPendingPharmacies();
                case "UPDATE_PHARMACY": return handleUpdatePharmacy(root);
                case "DELETE_PHARMACY": return handleDeletePharmacy(root);
    ...
    private String handleRegister(Element req) {
        ...
        String contactNumber = getText(req, "contactNumber");
        String address = getText(req, "address");
        // Create user as PENDING (isActive = false) - requires admin approval
        User user = dataStore.createUser(username, password, fullName, email, contactNumber, address, UserType.RESIDENT, -1, false);
        if (user == null) {
            server.log(username, "REGISTER_FAILED", "Username '" + username + "' already exists");
            return errorResponse("Username already exists");
        }
        server.log(username, "REGISTER", "New resident account created (PENDING): " + fullName);
        return successResponse("Registration successful! Your account is pending admin approval.", userToXml(user));
    }
    ...
    private String handleCreateUser(Element req) {
        ...
        // Admin-created users are ACTIVE immediately (isActive = true)
        User user = dataStore.createUser(username, password, fullName, email, contactNumber, address,
            UserType.valueOf(userType), pharmacyId, true);
        if (user == null) {
    ...
    private String handleUpdateUser(Element req) {
        ...
        String email = getText(req, "email");
        String password = getText(req, "password");
        boolean ok = dataStore.updateUser(userId, username, fullName, email, password);
        if (!ok) return errorResponse("Failed to update user. Username may already be taken.");
    ...
    private String handleGetPendingUsers() {
        List<User> users = dataStore.getPendingUsers();
        StringBuilder data = new StringBuilder("<users>");
        for (User u : users) data.append(userToXml(u));
        data.append("</users>");
        server.log(currentUsername, "GET_PENDING_USERS", "Retrieved " + users.size() + " pending users");
        return successResponse("OK", data.toString());
    }

    private String handleApproveUser(Element req) {
        int userId = parseInt(getText(req, "userId"), -1);
        boolean ok = dataStore.approveUser(userId);
        if (!ok) return errorResponse("Failed to approve user");
        server.log(currentUsername, "APPROVE_USER", "Approved user ID: " + userId);
        return successResponse("User approved", "");
    }

    private String handleRejectUser(Element req) {
        int userId = parseInt(getText(req, "userId"), -1);
        boolean ok = dataStore.rejectUser(userId);
        if (!ok) return errorResponse("Failed to reject user");
        server.log(currentUsername, "REJECT_USER", "Rejected user ID: " + userId);
        return successResponse("User rejected", "");
    }
    ...
    private String handleGetPendingPharmacies() {
        List<Pharmacy> list = dataStore.getPendingPharmacies();
        StringBuilder data = new StringBuilder("<pharmacies>");
        for (Pharmacy p : list) data.append(pharmacyToXml(p));
        data.append("</pharmacies>");
        return successResponse("OK", data.toString());
    ...
    private String handleUpdatePharmacy(Element req) {
        int pharmacyId = parseInt(getText(req, "pharmacyId"), -1);
        String name = getText(req, "name");
        String address = getText(req, "address");
        String contactNumber = getText(req, "contactNumber");
        String email = getText(req, "email");
        String description = getText(req, "description");
        boolean ok = dataStore.updatePharmacy(pharmacyId, name, address, contactNumber, email, description);
        if (!ok) return errorResponse("Failed to update pharmacy");
        server.log(currentUsername, "UPDATE_PHARMACY", "Updated pharmacy ID: " + pharmacyId);
        return successResponse("Pharmacy updated", "");
    }

    private String handleDeletePharmacy(Element req) {
        int pharmacyId = parseInt(getText(req, "pharmacyId"), -1);
        boolean ok = dataStore.deletePharmacy(pharmacyId);
        if (!ok) return errorResponse("Failed to delete pharmacy");
        server.log(currentUsername, "DELETE_PHARMACY", "Deleted pharmacy ID: " + pharmacyId);
        return successResponse("Pharmacy deleted", "");
    }
    ...
    private String userToXml(User u) {
        return "<user>" +
            ...
            "<address>" + esc(u.getAddress()) + "</address>" +
